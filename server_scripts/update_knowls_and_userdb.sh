...
then
  ...
  time pg_restore --host /var/run/postgresql --single-transaction --clean --if-exists --dbname lmfdb $userdbdump
  time pg_restore --host /var/run/postgresql --single-transaction --clean --if-exists --dbname lmfdb $knowlsdump
  time psql --dbname lmfdb --command "BEGIN; CREATE TEMP TABLE tmp (name text,  table_description text, col_description jsonb); COPY tmp FROM '$descdump' (FORMAT csv); UPDATE meta_tables SET (table_description, col_description) = (tmp.table_description, tmp.col_description) FROM tmp WHERE meta_tables.name = tmp.name; DO language plpgsql \$\$ BEGIN RAISE NOTICE 'updating meta_tables done'; END \$\$; END;"

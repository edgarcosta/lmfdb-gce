...
then
  ...
  time psql --dbname lmfdb --command "\\set AUTOCOMMIT false\nCREATE TEMP TABLE tmp (name text,  table_description text, col_description jsonb); COPY tmp FROM '$descdump' (FORMAT csv); UPDATE meta_tables SET (table_description, col_description) = (tmp.table_description, tmp.col_description) FROM tmp WHERE meta_tables.name = tmp.name; COMMIT;"

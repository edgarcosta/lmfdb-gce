/*
   Same as dir_test1.cpp but use callback interface instead.
*/
...
int main (int argc, char**argv)
{
  ...
  printf("Needs a_n for n <= %ld, but only giving n <= %lu\n", (long)Lfunc_nmax(L), (long unsigned)bound);
  ...
  for(int i  = 0; !arb_is_zero(zeros+i); ++i) {
    printf("Zero %d = ", i);
